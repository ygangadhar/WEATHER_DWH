/****** This SP is to Truncate and load [FCT].[3HourlyData] table ******/
/****** Execute it by running EXEC [dbo].[LoadFct3HourlyData] ******/
...
BEGIN
     ...
     SELECT  ISNULL(CTY.CitiSK,-1) AS CitiSK
			...
			, CASE WHEN CONVERT(NVARCHAR(8),DATEADD(SS,F3H.[TimeofDataForecasted],'19700101 00:00:00:000'),112) 
                        +LEFT(CONVERT(NVARCHAR(8),DATEADD(SS,F3H.[TimeofDataForecasted],'19700101 00:00:00:000'),114),2) > 
				    CONVERT(NVARCHAR(8),GETDATE(),112)+LEFT(CONVERT(NVARCHAR(8),GETDATE(),114),2) THEN 1 ELSE 0
			  END AS IsForecastFuture
